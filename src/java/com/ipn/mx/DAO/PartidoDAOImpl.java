...
import com.ipn.mx.Modelo.PartidoKey;
...
public class PartidoDAOImpl {
    ...
    private static final String SQL_SELECT_PROXIMAS_FECHAS
            = "SELECT p.fecha,p.hora,p.equipoLocal,p.equipoVisitante,e.estadio FROM Partido p, Equipo e "
            + "WHERE (e.nombre like equipoVisitante OR e.nombre like equipoLocal)"
            + "AND (equipoLocal like ? OR equipoVisitante like ?)"
            + " AND p.temporada = ?  AND p.partidoFinalizado = ?";
    ...
    public List<Object[]> loadProximosPartidos(String temporada, String equipo, Connection conn) throws SQLException {
        ...
        try {
            ...
            ps.setInt(3, Integer.valueOf(temporada));
